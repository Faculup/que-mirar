...
import { Category, Casa } from '../../../models/casa.model';
...
export class CasaComponent implements OnInit {
  casas$: Observable<Category[]> = of([]);
  rooms$: Observable<Room[]> = of([]);
  selectedCasa: Category | null = null;
  ...
  isCategoryHome(): boolean {
    return (
      this.selectedCasa?.name?.toLowerCase() === 'home' ||
      (this.selectedCasa?.isHouse ?? false)
    );
  }
  ...
  deleteCategory(category: Category): void {
    ...
    // If this is the currently selected category, deselect it
    if (this.selectedCasa?.id === category.id) {
      this.convivenciaService.clearSelections();
